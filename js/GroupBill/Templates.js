import { getInfoDB } from '../utils/FirebaseUtils.js'

export function getTemplateParticipants (participantsList) {
...
export async function getTemplateUserCreatedExpenses (amount) {
  try {
    const expenses = await getInfoDB(`expenses/`)
    let totalIncome = 0
        ...
        ([uid, { ownerEmail, name, totalAmount, participantsHaveToPay }]) => {
          ...
          <li class="user-created-expenses__item" data-expense-id="${uid}">
            ...
            <div class="user-created-expenses__button-container">
              <button class="user-created-expenses__delete expense-control-btn" data-action="delete">${
            document.documentElement.lang === 'ru'
              ? 'Удалить'
              : 'Delete'
          }</button>
              <button class="user-created-expenses__edit expense-control-btn" data-action="edit">${
            document.documentElement.lang === 'ru'
              ? 'Редакт.'
              : 'Edit'
          }</button>
...
export async function getTemplateExpensesToPay (amount) {
  try {
    const expenses = await getInfoDB(`expenses/`)
    ...
    return [
      `<li class="expense-to-pay__item" data-expense-id="not-an-item">${
        document.documentElement.lang === 'ru'
          ? 'Нет счетов'
          : 'No expenses'}</li>`,
      0,
...
export function getInfoParticipant (participantsList) {
  return Object.entries(participantsList).map(([email, { sum, hasPaid }]) => {
...
export function getTemplateRecentPayments (recentPaymentsList, amount) {
